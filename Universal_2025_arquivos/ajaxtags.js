...
AjaxJspTag.UpdateField.prototype = Object.extend(new AjaxJspTag.Base(), {
  ...
  execute: function(e) {
    if (this.options.preFunction != null) {
      retorno = this.options.preFunction();
...
AjaxJspTag.HtmlContent.prototype = Object.extend(new AjaxJspTag.Base(), {
  ...
  setListeners: function() {
    if (this.options.source) {
      eval("$(this.options.source).on"+this.options.eventType+" = this.execute.bindAsEventListener(this)");
        ...
        var classes = this.options.sourceClass;
        if (classes.indexOf(";") != -1) {
          var classesArray = classes.split(";");
          for (var i=0; i<classesArray.length; i++) {  
              var elementArray = document.getElementsByClassName(classesArray[i]);        
              for (var j=0; j<elementArray.length; j++) {
                eval("elementArray[j].on"+this.options.eventType+" = this.execute.bindAsEventListener(this)");
              }
          }
        } else {
  ...
  execute: function(e) {
    if (this.options.preFunction != null) {
      retorno = this.options.preFunction(e);
    ...
    var aj = new Ajax.Updater(this.options.target, this.url, {
      ...
      onFailure: function(request) {
        if (obj.options.errorFunction != null) {
            obj.options.errorFunction(request);
        }
      },
      onComplete: function(request) {
        if (obj.options.postFunction != null) {
            obj.options.postFunction(request);
        }
      }
    });
  }
...
AjaxJspTag.Callout.prototype = Object.extend(new AjaxJspTag.Base(), {
  ...
  execute: function(e) {
    ...
    if (elem.type) {
      ...
      ajaxParameters = ajaxParameters.replace(re, valor);
    } else {
      ajaxParameters = ajaxParameters.replace(re, elem.innerHTML);
    }
    var params = buildParameterString(ajaxParameters);
